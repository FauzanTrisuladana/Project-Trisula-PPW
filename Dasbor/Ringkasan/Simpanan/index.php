...
// Mulai session
startSession();
$asal = 'Dasbor/Ringkasan/Simpanan';
if (!isLoggedIn()) {
...
$totalQuery = "SELECT COUNT(*) as total FROM view_total_simpanan_per_anggota WHERE id_koperasi = ? AND (id_custom_anggota LIKE ? OR nama_anggota LIKE ?)";
...
$query = "SELECT total_pokok, total_wajib, tanggal_terakhir_wajib, total_sukarela, id_custom_anggota, nama_anggota FROM view_total_simpanan_per_anggota WHERE id_koperasi = ? AND (id_custom_anggota LIKE ? OR nama_anggota LIKE ?) LIMIT ? OFFSET ?";
$stmt = mysqli_prepare($conn, $query);
mysqli_stmt_bind_param($stmt, 'issii', $_SESSION['id_koperasi'], $search, $search, $perpageget, $offset);
...
<body class="mt-0">
    ...
    <main>
        <section class="d-flex justify-content-between">
            <div class="w-auto">
                <h1 class="jdl w-auto">Ringkasan Simpanan</h1>
                <div class="navatas">
                ...
                <table class="table table-hover table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>Rp. <?php echo number_format($row['total_pokok'], 2, ',', '.'); ?></td>
                            <td>Rp. <?php echo number_format($row['total_wajib'], 2, ',', '.'); ?></td>
                            <td><?php echo htmlspecialchars($row['tanggal_terakhir_wajib']); ?></td>
                            <td>Rp. <?php echo number_format($row['total_sukarela'], 2, ',', '.'); ?></td>
                            <td>Rp. <?php echo number_format($row['total_pokok']+$row['total_wajib']+$row['total_sukarela'], 2, ',', '.'); ?></td>
                        </tr>
                        <?php endwhile; ?>
